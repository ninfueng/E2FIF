...
class checkpoint:
    ...
    def save_results(self, dataset, filename, save_list, scale):
        if self.args.save_results:
            ...
            mid_feas = (
                mid_feas.squeeze(0).mul(255 / self.args.rgb_range).cpu().clamp(0, 1)
                * 255
            )
            mid_feas_sign = (
                mid_feas_sign.squeeze(0)
                .mul(255 / self.args.rgb_range)
                .cpu()
                .clamp(0, 1)
                * 255
            )
    ...
    def save_results_my(self, dataset, filename, save_list, scale, args):
        if self.args.save_results:
            ...
            psnr_bicubic, ssim_bicubic = new_psnr(
                hr_ycbcr[:, :, 0],
                sr_bicubic,
                scale=scale,
...
def get_rgb(y, ycbcr):
    ...
    rgb_show_tangle = cv2.rectangle(
        rgb_show_tangle, left_up, right_bottom, (0, 255, 0), 4
    )

    toup_img = rgb_show_tangle[
        left_up[1] : right_bottom[1], left_up[0] : right_bottom[0], :
    ]
