        ...
        for batch, (
            ...
            if (batch + 1) % self.args.print_every == 0:
                ...
                self.ckp.tb_write_log(
                    ...
                    step=len(self.loader_train) * epoch + batch,
                    prefix="train",
                )
        ...
        for idx_data, d in enumerate(self.loader_test):
            ...
            for idx_scale, scale in enumerate(self.scale):
                ...
                for lr, hr, filename in tqdm(d, ncols=80):
                    ...
                    psnr = new_psnr(
                        hr, sr, scale=scale, data_range=self.args.rgb_range, chop=True
                    )
                    ssim = new_ssim(
                        hr, sr, scale=scale, data_range=self.args.rgb_range, chop=True
                    )
                    ...
                    if (
                        self.args.save_results
                        and dataset_name == "test"
                        and filename[0] == "img_004"
                    ):
                        # if self.args.save_results and dataset_name == 'test' and filename[0] == 'baboon':
                        # if self.args.save_results:
                        print(dataset_name, filename)
                        # self.ckp.save_results(d, filename[0], save_list, scale)
                        psnr_bicubic, ssim_bicubic = self.ckp.save_results_my(
                            d, filename[0], save_list, scale, self.args
                        )
                ...
                self.ckp.tb_write_log(
                    ...
                    step=epoch,
                    prefix="test_{}".format(dataset_name),
                )
